...
# LTL task formula
is_surveillance = True   # !!!!!!!!!! ATTENTION !!!!!!!!!
...
single_init_path_length = {'whole_path': float(
    "inf"), 'pre_path': float("inf"), 'suf_path': float("inf")}
best_path = {'whole_path': [], 'pre_path': [], 'suf_path': []}
best_path_length = {'whole_path': float(
    "inf"), 'pre_path': float("inf"), 'suf_path': float("inf")}
...
for i in range(0, len(best_path['pre_path'])):
    best_pre_path_trans.append(
        product_graph.node[best_path['pre_path'][i]]['ts_name'])
best_suf_path_trans = []
for i in range(0, len(best_path['suf_path'])):
    best_suf_path_trans.append(
        product_graph.node[best_path['suf_path'][i]]['ts_name'])
